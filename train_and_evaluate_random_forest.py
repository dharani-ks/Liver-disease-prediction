...
from sklearn.ensemble import RandomForestClassifier  # Changed from Decision Tree
...
X = pd.DataFrame(X_scaled, columns=dataset_imputed.drop('Dataset', axis=1).columns)  # Keep column names
...
# Step 10: Train the model (Random Forest Classifier)
model = RandomForestClassifier(n_estimators=100, random_state=42)  # Replaced Decision Tree with Random Forest
...
model_status = [["Model", "RandomForestClassifier"],
...
print(f"Total number of records in the dataset: {len(dataset)}")
